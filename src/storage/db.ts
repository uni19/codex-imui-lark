...
export function createMemoryStore(): Store {
  ...
  const sessionByCodexId = new Map<string, ImSession>()
  ...
  return {
    ...
    async get_session_by_codex(session_id) {
      return sessionByCodexId.get(session_id) ?? null
    },
    ...
    async save_session(input) {
      ...
      const hit = sessionByCodexId.get(input.session_id)
      if (old && old.session_id !== input.session_id) sessionByCodexId.delete(old.session_id)
      if (hit) {
        sessionByMapKey.delete(key(hit.tenant_id, hit.chat_id, hit.thread_id))
      }
      sessionByMapKey.set(mapKey, input)
      sessionByCodexId.set(input.session_id, input)
    },
...
export function createSqliteStore(file: string): Store {
  ...
  const getSessionByCodexStmt = db.query<{ data: string }, [string]>(
    "select data from im_session where codex_session_id = ?1 limit 1",
  )
  const getSessionMapStmt = db.query<{ codex_session_id: string | null }, [string]>(
    "select codex_session_id from im_session where map_key = ?1 limit 1",
  )
  ...
  const dropSessionStmt = db.query("delete from im_session where codex_session_id = ?1")
  const getSessionModelPrefStmt = db.query<{ data: string }, [string]>(
    "select data from session_model_pref where codex_session_id = ?1 limit 1",
  )
  const saveSessionModelPrefStmt = db.query(
    `
      insert into session_model_pref (codex_session_id, data, updated_at)
      values (?1, ?2, ?3)
  ...
  const dropSessionModelPrefStmt = db.query("delete from session_model_pref where codex_session_id = ?1")
  ...
  return {
    ...
    async get_session_by_codex(session_id) {
      return parse<ImSession>(getSessionByCodexStmt.get(session_id)?.data ?? null)
    },

    async save_session(input) {
      if (!live) return
      const mapKey = key(input.tenant_id, input.chat_id, input.thread_id)
      const old = getSessionMapStmt.get(mapKey)?.codex_session_id
