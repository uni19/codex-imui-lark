...
import type { ChatType, ImSession, CodexSvc, SessionModelPref, SessionSvc, Store } from "../contracts.js"
...
async function materialize_pending(input: Input, row: ImSession): Promise<ImSession> {
  const result = await input.codex.ensure({
...
export function createSessionSvc(input: Input): SessionSvc {
  return {
    ...
    async switch(val) {
      ...
      const known = await input.store.get_session_by_codex(val.session.id)
    ...
    async bind(val) {
      const items = await input.store.get_session_by_codex(val.session_id)
      ...
      await input.store.save_session(next)
      if (next.session_id !== items.session_id) {
    ...
    async model(val) {
      const item = await input.store.get_session_by_codex(val.session_id)
      if (!item) return null
