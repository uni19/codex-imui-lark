import type { AppCfg, ConnState, ImSession, CodexModel, CodexResult, RepoPref, Task } from "../contracts.js"
import { escapeDynamicMarkdownText } from "../feishu/api.js"
import { normalizeWorkspace } from "../workspace.js"

export type RecoverMode = "boot" | "message" | "codex"
...
export function model(val?: CodexModel) {
  if (!val) return "未设置"
  const prefix = val.providerID ? `${val.providerID}/` : ""
  return val.variant ? `${prefix}${val.modelID}@${val.variant}` : `${prefix}${val.modelID}`
}
...
function next(
  ...
  if (codex?.status === "reconnecting" || codex?.status === "error") {
    return "当前正在等待 Codex 连接恢复，可稍后重试 /status，或发送 /abort 终止。"
  }
...
export function done_msg(val: CodexResult) {
...
export function recover_msg(mode: RecoverMode, kind: "approval" | "question" | "attachment" | "cache" | "wait" | "sync" | "queued" | "running") {
  const head = mode === "boot" ? "服务" : mode === "message" ? "飞书消息连接" : "Codex 连接"
...
export function signal_msg(item: ConnState) {
  const head = item.name === "message" ? "飞书消息连接" : "与 Codex 的连接"
...
function status_lines(input: StatusInput) {
  return [
    ...
    `当前模型：${model(input.current?.model ?? input.conf.codex.model)}`,
    `默认模型：${model(input.conf.codex.model)}`,
    ...
    `Codex 连接：${conn(input.codex)}`,
    input.row ? `最近更新：${time(input.row.updated_at)}` : undefined,
    view(input.row) ? `最近进展：${view(input.row)}` : undefined,
    `下一步：${next(input.row, input.current, input.syncd, input.message, input.codex)}`,
...
export function status_card(input: StatusInput) {
  const [title = "Codex", ...body] = status_lines(input)
...
export function friendly(err: unknown): string {
  ...
  if (
    ...
    low.includes("codex request failed:")
  ) {
  ...
  if (low.includes("codex request failed: 404")) return "Codex 接口不可用：请检查服务地址、接口版本或 base_url 配置。"
  if (low.includes("codex request failed: 429") || low.includes("rate limit") || low.includes("too many requests")) {
  ...
  if (low.includes("invalid_api_key") || low.includes("api key not valid")) {
    return "认证失败：请检查 Codex 或飞书的账号、密码和权限配置。"
  }
  if (
  ...
  if (
    low.includes("codex request failed: 500") ||
    low.includes("codex request failed: 502") ||
    low.includes("codex request failed: 503") ||
    low.includes("codex request failed: 504")
  ) {
    return "Codex 服务异常：服务端暂时不可用，请稍后重试。"
  }
  if (low.includes("codex request failed: 400")) {
...
function advice(err: unknown) {
  ...
  if (low.includes("workspace not found:") && low.includes("codex request failed:")) {
  ...
  if (
    low.includes("codex request failed: 429") ||
  ...
  if (
    ...
    low.includes("unsupported model") ||
    low.includes("invalid model") ||
    low.includes("codex request failed: 404")
  ) {
