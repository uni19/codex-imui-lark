...
export function renderPackageReadme(version: string, target: Target) {
  return [
    ...
    "- `FEISHU_API_BASE_URL`：OpenAPI 根地址，默认 `https://open.feishu.cn/open-apis`；Lark global 可改为 `https://open.larksuite.com/open-apis`，长连接 SDK domain 会自动跟随为 `https://open.larksuite.com`。",
    "- `FEISHU_WS_BASE_URL`：长连接 SDK domain，默认从 `FEISHU_API_BASE_URL` 推导；私有化或代理环境可显式覆盖。",
    "- `FEISHU_WS_ENDPOINT_URL`：长连接 endpoint 完整 URL，默认是 `FEISHU_WS_BASE_URL + /callback/ws/endpoint`；当 OpenAPI 和长连接 endpoint 不同源时可显式覆盖。",
