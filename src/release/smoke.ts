...
export function serviceFile(home: string, platform = process.platform) {
  if (platform === "darwin") return path.join(home, "Library", "LaunchAgents", "com.codex-feishu-imui.plist")
  if (platform === "linux") return path.join(home, ".config", "systemd", "user", "codex-feishu-imui.service")
  return fail(`unsupported smoke platform: ${platform}`)
}
...
export async function smoke(root = process.cwd(), argv = process.argv) {
  ...
  const work = await mkdtemp(path.join(tmpdir(), "codex-feishu-smoke-"))
  ...
  try {
    ...
    const base = items.find((item) => item.startsWith("codex-feishu-imui-") && !item.endsWith(".tar.gz"))
    if (!base) fail("smoke unpack failed: package directory not found")
    const stage = path.join(work, base)
    const bin = path.join(prefix, "bin", "codex-feishu-imui")
    const service = path.join(prefix, "bin", "codex-feishu-imui-service")
