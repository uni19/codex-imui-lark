...
一个把 Codex 接到飞书 IM 的网关服务。

它负责接收飞书私聊 / 群聊消息，把文本、图片、文件和富文本转换成 Codex 可消费的输入，再把执行进度、审批、追问和最终结果回推到飞书。

当前仓库已经不是纯方案稿，已经包含可运行的 Bun 服务、SQLite 持久化、飞书长连接接入、Codex app-server JSON-RPC 对接，以及一批围绕恢复、队列、命令和多模态的自动化测试。
...
- 上传图片、文件或发送 `post` 富文本，让 Codex 基于附件回答
- 在飞书里处理 Codex 的权限审批和问题回问
...
- Bun 1.x
- 已登录可用的 Codex CLI（服务会自行启动 `codex app-server --stdio`）
...
- `CODEX_DIRECTORY` 建议填绝对路径，作为默认工作目录，不要求是 Git worktree
- 当前 Codex app-server 适配以本地目录线程为主；`CODEX_WORKSPACE` 预留兼容，通常留空

### 4. 启动服务

确认 `codex` 命令已登录可用后启动 IMUI：
...
如果 Codex CLI、数据库和目录配置都正确，服务会开始创建任务并输出处理结果。
...
- `docs/04-api-contracts.md`: 网关和 Codex 的接口契约
...
- 不改 Codex 内核协议，优先通过 `codex serve` 暴露的 HTTP API 和 SSE 事件实现
- IM 接入层做成独立网关服务，负责飞书长连接、协议翻译、幂等、消息卡片和状态回推
- SDK 优先使用 `@codex-ai/sdk/v2`，因为它包含 `permission` 和 `question` 能力，适合 IM 审批回环
...
- 已支持飞书图片、文件和 `post` 富文本消息；多图、图文混排会统一拆成 `text + file parts`，附件-only 输入会先进入“等待补充说明”，用户补一条文本后再一起送入 Codex
...
- `/repo`: 查看当前目录绑定
- `/repo <directory>`: 为当前会话绑定目录；若目录变更，会同步切换到新的 Codex session
...
- 未命中的 slash 会尝试透传到 Codex 执行，例如 `/init`
...
- 平台对应的单文件二进制
- 解压根目录的 `README.md`（面向安装用户的配置与使用说明）
- `install.sh`
- `uninstall.sh`
- `codex-feishu-imui-service` 服务助手
...
- 图片、文件和 `post` 富文本中的图片 / 文件节点会下载为本地缓存文件，再作为 Codex `file part` 输入
- 只有附件、没有文字时，bot 会先要求用户补一句“想让我做什么”
- 等待权限审批时，请直接点击卡片按钮；如需更正当前操作，可直接发送非数字文本说明，系统会按“更正当前操作并继续执行”处理
- 等待问题追问时，如存在选项，请优先在卡片中选择后提交；若该问题允许自定义回答，可直接发送非数字文本；若没有选项，可直接发送回答继续

## 关键参考

- Codex Server: https://codex.ai/docs/zh-cn/server/
- Codex SDK: https://codex.ai/docs/zh-cn/sdk/
- Codex Web: https://codex.ai/docs/zh-cn/web/
- Codex 源码: 独立仓库 `codex`
