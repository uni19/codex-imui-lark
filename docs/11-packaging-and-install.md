...
- 安装后默认使用 `~/.config/codex-feishu-imui/.env` 作为配置文件
...
```text
dist/release/
  codex-feishu-imui-<version>-<target>.tar.gz
```
...
- `bin/codex-feishu-imui`: 编译后的真实二进制
- `README.md`: 面向最终用户的安装、配置和使用说明
- `install.sh`: 安装脚本
- `uninstall.sh`: 卸载脚本
...
- `share/codex-feishu-imui.env.example`: 安装态配置模板
...
- 把真实二进制复制到 `~/.local/lib/codex-feishu-imui/`
- 生成启动包装器 `~/.local/bin/codex-feishu-imui`
- 生成服务助手 `~/.local/bin/codex-feishu-imui-service`
- 生成配置目录 `~/.config/codex-feishu-imui`
- 生成数据目录 `~/.local/share/codex-feishu-imui`
- 若不存在配置文件，则创建 `~/.config/codex-feishu-imui/.env`
...
2. `IMUI_ENV_FILE`
3. 当前工作目录下的 `.env`
4. `~/.config/codex-feishu-imui/.env`

安装脚本生成的包装器会显式传入：

```text
--env-file ~/.config/codex-feishu-imui/.env
```
...
- Linux 建议用 `systemctl --user status codex-feishu-imui` 和 `journalctl --user -u codex-feishu-imui` 查看
...
3. 执行 `codex-feishu-imui --help`
4. 确认 `~/.config/codex-feishu-imui/.env` 已生成
5. 回到源码仓库，用安装态配置再跑一遍 `bun run release:doctor -- --env-file ~/.config/codex-feishu-imui/.env`
6. 如需要常驻运行，再执行一次 `codex-feishu-imui-service install`
...
```bash
bun run release:gate -- --env-file ~/.config/codex-feishu-imui/.env
```
