...
- [ ] `CODEX_BASE_URL` 已指向可访问的 Codex Server
- [ ] `CODEX_PASSWORD` 已配置且和服务端一致
- [ ] `CODEX_DIRECTORY` 已配置且目录真实存在
- [ ] 若要验证远端 workspace：`CODEX_WORKSPACE` 已配置为可用 `wrk*` ID；若验证本地项目，保持为空
- [ ] `IMUI_DB_PATH` 所在目录可写
...
- [ ] 安装后的 `codex-feishu-imui --help` 可执行
- [ ] 安装后的 `codex-feishu-imui-service install` / `uninstall` 可执行
- [ ] 安装后会生成默认配置文件 `~/.config/codex-feishu-imui/.env` 或目标 `CONFIG_DIR/.env`
