...
本文描述 OMO 目标合约，而不是当前代码已经完整提供的接口清单。现有外部 Codex / Feishu API 基本不变，主要变化发生在 Gateway 内部语义、存储模型和 runtime 对 `idle` 的解释方式上。
...
目标设计不要求改 Codex 原子接口，仍然围绕这些调用组织：
...
Gateway 仍然消费 Codex `/event` 或 `/global/event`，但语义改成：
...
保存 Feishu thread 到 foreground Codex session 的映射。
...
- 多个开放 user turn 不能共享一个 Codex `session_id`
