...
```text
Codex -> permission.asked / question.asked
...
Gateway -> Codex permission.reply / question.reply
Codex -> continue execution on same task
```
...
Gateway -> Codex session.prompt_async
```
...
Gateway -> create fresh Codex session B
...
- 新普通 prompt 不能和旧 live nonterminal task 共享同一个 Codex session
...
Codex -> session.error / idle / abort completion
...
```text
Message conn / Codex SSE -> reconnecting
