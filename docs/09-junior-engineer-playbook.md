...
- 图片、文件、`post` 富文本、多图、图文混排可进入 Codex
- SQLite 持久化已接入运行链路
...
- 回复提纯 / 最终结果抓取 -> `test/codex-client.test.ts`
...
- 权限审批通过卡片按钮处理；非数字文本视为更正说明
- 问题选项通过卡片提交；允许自定义补充时使用非数字文本
- 附件-only 先进入等待说明
- 群聊首条消息需要 `@bot`
- 连接抖动提示要避免刷屏
...
- boot 首次 `recover()`、后续 `on_conn(codex ready)`、watchdog `sweep()` 三者之间是否还存在重复提示或状态不一致
...
- boot 后 `queued` 任务在 Codex ready 后被重新确认并推进
- boot 后 `running` 任务在 Codex ready 后自动收尾
...
`queue_job` 落盘、Codex 事件基础幂等、迟到事件保护已经有了第一层实现，但还没有完全收口。
...
1. 明确“重复入站事件”“重复 Codex 事件”“迟到完成事件”“迟到权限事件”四类场景
2. 逐类补测试
...
- 同一条飞书消息重复入队时不重复创建任务
- 同一条 Codex idle/error 迟到到达时不污染已完成任务
- 队列恢复后不会重复执行已完成 job
