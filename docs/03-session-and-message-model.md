...
`ImSession` 描述的是“当前这个 Feishu thread 前台绑定到哪个 Codex session”，它解决的是前台路由，而不是所有活跃任务的全局真相。
...
### 1. 一个 Codex session 同时最多承载一个开放用户轮次

OMO 目标模型不允许多个未收口 user turn 共用一个 Codex `session_id`。这不是实现洁癖，而是为了避免 reply 归属、idle 终态、wait 队列、result_hash 对账全部打结。
...
- 不能继续复用当前 `session_id`
- 必须 `route.reset(...)` 到 fresh Codex session
- 新 prompt 生成新 task
