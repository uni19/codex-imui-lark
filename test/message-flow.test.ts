...
import type { AppCfg, FeishuApi, InboundMessage, CodexResult, CodexStatus, CodexSvc, RenderOut } from "../src/contracts.ts"
...
function codex() {
  const prompts: Array<Parameters<CodexSvc["prompt"]>[0]> = []
  const aborts: Array<Parameters<CodexSvc["abort"]>[0]> = []
  const ensures: Array<Parameters<CodexSvc["ensure"]>[0]> = []
  let seq = 0
  const svc: CodexSvc = {
    async ensure(input) {
      ensures.push(input)
...
describe("message flow", () => {
  ...
  test("reuses the same session and task for resumable follow-up", async () => {
    ...
    let statuses: Record<string, CodexStatus> = {}
    let last: string | undefined
    ai.svc.status = async (_input: { directory?: string; workspace?: string }): Promise<Record<string, CodexStatus>> => statuses
    ...
    const detached = (await store.list_tasks()).find((item) => item.id !== first.id)
    const detachedHistory = await store.list_assistant_outbounds(detached?.id ?? "")
    expect(detachedHistory.at(-1)).toMatchObject({
      kind: "intermediate",
      action: "reply",
  ...
  test("intermediate task follow-up keeps original status card as terminal patch target", async () => {
    ...
    let statuses: Record<string, CodexStatus> = {}
    let result: CodexResult = { state: "empty" }
    ai.svc.status = async (_input: { directory?: string; workspace?: string }): Promise<Record<string, CodexStatus>> => statuses
    ai.svc.result = async (_input: { session_id: string; directory?: string; workspace?: string }): Promise<CodexResult> => result
    ...
    expect(finished).toMatchObject({
      ...
      status_outbound_id: undefined,
      terminal_kind: "final",
      terminal_outbound_id: "out_reply_4",
      status: "completed",
    })

    expect(ui.replies.map((item) => item.id)).toEqual(["out_reply_1", "out_reply_2", "out_reply_3", "out_reply_4"])
    ...
    const history = await store.list_assistant_outbounds(first.id)
    expect(history.map((item) => item.kind)).toEqual(["ack", "final"])
    expect(history[0]).toMatchObject({
      kind: "ack",
      action: "reply",
    ...
    const detachedHistory = await store.list_assistant_outbounds(detached?.id ?? "")
    expect(detachedHistory.map((item) => item.kind)).toEqual(["ack", "intermediate", "intermediate"])
    expect(detachedHistory[0]).toMatchObject({
      kind: "ack",
      action: "reply",
  ...
  test("aborts running task and patches original status card", async () => {
    const store = createMemoryStore()
    const task = createTaskSvc(store)
    const ui = feishu()
    const ai = codex()
    ...
    const running = await store.get_last_task("ses_1")
    if (!running) throw new Error("missing running task")
    expect(running.status).toBe("running")
    expect(running.outbound_id).toBe("out_reply_1")
    ...
    await on_msg(
      ...
      render,
      ai.svc,
      inbound("in_run_abort", {
        text: "/abort",
      }),
    )

    const aborted = await store.get_task(running.id)
    if (!aborted) throw new Error("missing aborted task")
    expect(aborted.status).toBe("aborted")
    expect(aborted.note).toBe("已发送取消请求。")
    expect(ai.aborts).toEqual([{ session_id: "ses_1", directory: "/tmp", workspace: undefined }])
    expect(ui.patches).toHaveLength(1)
    expect(ui.patches[0]).toMatchObject({
      msg_id: "out_reply_1",
      out: {
