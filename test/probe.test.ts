...
function codex(input: { status?: Record<string, CodexStatus>; last?: string | undefined; result?: CodexResult; fail?: boolean }) {
  return {
    async ensure() {
      return { id: "ses_1" }
    },
...
describe("probe", () => {
  ...
  test("preserves unscoped runtime workspace for downstream probe calls", async () => {
    ...
    const oc = {
      ...
      async result(input: { session_id: string; directory?: string; workspace?: string }): Promise<CodexResult> {
        calls.push({
          name: "result",
          input: {
