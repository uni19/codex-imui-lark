...
describe("sweep", () => {
  ...
  test("shows reconnecting hint for stale running task when codex is reconnecting", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()
    ...
    expect(ui.list[ui.list.length - 1]?.out).toMatchObject({
      ...
      body: {
        template: "blue",
        text: "与 Codex 的连接暂时中断，正在重连…（第 2 次，约 4 秒后重试） 原因：网络请求失败：无法连接到服务，请检查服务地址、网络、代理或 TLS 配置。",
      },
    })
  })

  test("shows connecting hint for stale running task before codex conn is known", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()
