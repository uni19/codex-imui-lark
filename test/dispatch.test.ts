...
import type { FeishuApi, ImSession, InboundMessage, CodexEvent, CodexResult, CodexSvc, RenderOut, Task } from "../src/contracts.ts"
...
describe("dispatch_event", () => {
  test("flushes progress before repeated idle settlement", async () => {
    ...
    await dispatch_event(store, task, ui.api, render, codex("done"), stream, {
      type: "session.status",
      properties: {
  ...
  test("flushes progress before session error event", async () => {
    ...
    await dispatch_event(store, task, ui.api, render, codex(), stream, {
