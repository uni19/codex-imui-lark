...
describe("app env", () => {
  ...
  test("explicit argv env file overrides preloaded values and IMUI_ENV_FILE", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "codex-feishu-env-"))
  ...
  test("prefers cwd .env over config home fallback", async () => {
    ...
    const conf = path.join(home, ".config", "codex-feishu-imui")
  ...
  test("derives config and data directories from XDG or HOME", () => {
    expect(configDir({ cwd: "/repo", env: { XDG_CONFIG_HOME: "/tmp/xdg" } })).toBe("/tmp/xdg/codex-feishu-imui")
    expect(dataDir({ cwd: "/repo", env: { HOME: "/home/demo" } })).toBe("/home/demo/.local/share/codex-feishu-imui")
  })
})
