...
describe("error helpers", () => {
  test("maps common transport errors to user friendly text", () => {
    ...
    expect(friendly("codex request failed: 401 Unauthorized")).toBe(
      "认证失败：请检查 Codex 或飞书的账号、密码和权限配置。",
    )
    expect(friendly("codex request failed: 429 Too Many Requests - rate limit")).toBe(
    ...
    expect(friendly("codex request failed: 400 Bad Request - context length exceeded")).toBe(
      "模型请求失败：上下文过长，请缩短问题、减少附件，或拆成多轮发送。",
    )
    expect(friendly("codex request failed: 404 Not Found")).toBe(
      "Codex 接口不可用：请检查服务地址、接口版本或 base_url 配置。",
    )
    expect(friendly("codex request failed: 500 Internal Server Error - Workspace not found: wrk_missing")).toBe(
  ...
  test("adds retry vs resend guidance for common runtime errors", () => {
    expect(explain("codex request failed: 429 Too Many Requests - rate limit")).toContain(
      "建议：可稍后重试当前请求，一般不需要重发上一条消息。",
    )
    expect(explain("feishu asset failed: 404 Not Found")).toContain(
      "建议：请重新发送附件和说明，再试一次。",
    )
    expect(explain("codex request failed: 500 Internal Server Error - Workspace not found: wrk_missing")).toContain(
      "建议：先发送 /workspaces 确认可用 workspace ID；如果你要用本地项目，请省略 --workspace；如果你要清空当前绑定，直接发送 /repo --workspace。",
    )
    expect(explain("codex request failed: 400 Bad Request - session state invalid")).toContain(
    ...
    expect(
      status_text({
        row: {
          ...
          status: "failed",
          err: "codex request failed: 429 Too Many Requests - rate limit",
        },
        current: null,
        pref: {
          chat: null,
