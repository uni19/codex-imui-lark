...
describe("release smoke", () => {
  ...
  test("maps service manager and file by platform", () => {
    ...
    expect(serviceFile("/tmp/demo", "darwin")).toBe(path.join("/tmp/demo", "Library", "LaunchAgents", "com.codex-feishu-imui.plist"))
    expect(serviceFile("/tmp/demo", "linux")).toBe(path.join("/tmp/demo", ".config", "systemd", "user", "codex-feishu-imui.service"))
  })
  ...
  test("derives default artifact from package version and current target", async () => {
    ...
    expect(out).toContain(path.join("dist", "release", "codex-feishu-imui-0.1.0-"))
