...
function file() {
  return path.join("/tmp", `codex-feishu-imui-${crypto.randomUUID()}.sqlite`)
}
...
describe("sqlite store", () => {
  test("persists runtime state across reopen", async () => {
    ...
    expect(await b.get_session_by_codex("ses_1")).toMatchObject(ses)
  ...
  test("replaces a persisted pending_new placeholder with a real active session on the same map key", async () => {
    ...
    expect(await b.get_session_by_codex(placeholder.session_id)).toMatchObject(placeholder)

    await b.save_session(active)

    expect(await b.get_session({ tenant_id: "tenant", chat_id: "chat" })).toMatchObject(active)
    expect(await b.get_session_by_codex(placeholder.session_id)).toBeNull()
    expect(await b.get_session_by_codex(active.session_id)).toMatchObject(active)
    await b.close?.()

    const c = createSqliteStore(db)
    expect(await c.get_session({ tenant_id: "tenant", chat_id: "chat" })).toMatchObject(active)
    expect(await c.get_session_by_codex(placeholder.session_id)).toBeNull()
    expect(await c.get_session_by_codex(active.session_id)).toMatchObject(active)
    await c.close?.()
  })
  ...
  test("keeps only the latest session mapping", async () => {
    ...
    expect(await db.get_session_by_codex("ses_old")).toBeNull()
    ...
    expect(await db.get_session_by_codex("ses_new")).toMatchObject(moved)
    await db.close?.()
  })
