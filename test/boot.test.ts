...
function cfg(input?: Partial<AppCfg["codex"]>) {
  return {
    log: { level: "info" },
    storage: { path: ":memory:" },
    feishu: { mode: "off" },
...
describe("boot helpers", () => {
  ...
  test("renders status report with connection and progress summary", () => {
    ...
    ).toContain("Codex 连接：reconnecting")
    expect(
      status_text({
        row: row({
          status: "waiting_question",
    ...
    ).toContain("Codex 连接：reconnecting #2 - 约 4 秒后重试 - 网络请求失败：无法连接到服务，请检查服务地址、网络、代理或 TLS 配置。")
    expect(
      status_text({
        row: row({
          status: "waiting_question",
    ...
    ).toContain("下一步：当前正在等待 Codex 连接恢复，可稍后重试 /status，或发送 /abort 终止。")
  })
  ...
  test("codex reconnect runs recover path", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()
    await store.save_session(session())
    ...
    expect(ui.list[ui.list.length - 1]?.out).toMatchObject({
      ...
      body: {
        template: "blue",
        text: "Codex 连接已恢复，正在继续同步执行状态…",
      },
    })
  })

  test("codex first ready after connecting also runs recover path", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()
    await store.save_session(session())
  ...
  test("boot sync hint is patched forward when codex becomes ready", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()
    await store.save_session(session())
  ...
  test("boot recovered queued task is not failed again after codex first ready", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()
    await store.save_session(session())
  ...
  test("boot recovered running task checkpoints first idle observation, settles once, and ignores later recovery passes", async () => {
    ...
    await recover(cfg(), store, svc, ui.api, render, codex({ status: null }), "boot")
    await on_conn(
      cfg(),
      store,
      svc,
  ...
  test("connection burst only signals reconnecting once for active task", async () => {
    ...
    expect(ui.list[0]?.out).toMatchObject({
      ...
      body: {
        template: "blue",
        text: "与 Codex 的连接暂时中断，正在重连…（第 1 次，约 1 秒后重试） 原因：网络请求失败：无法连接到服务，请检查服务地址、网络、代理或 TLS 配置。",
      },
    })
  })
  ...
  test("reconnecting reason change updates active task immediately", async () => {
    ...
    expect(ui.list[ui.list.length - 1]).toMatchObject({
      ...
      out: {
        ...
        body: {
          template: "blue",
          text: "与 Codex 的连接暂时中断，正在重连…（第 2 次，约 2 秒后重试） 原因：网络请求失败：证书校验失败，请检查代理、HTTPS 证书或企业网关配置。",
        },
      },
    })
  })
  ...
  test("new reconnect cycle after recovery is not hidden by cooldown", async () => {
    ...
    expect(ui.list[ui.list.length - 1]).toMatchObject({
      ...
      out: {
        ...
        body: {
          template: "blue",
          text: "与 Codex 的连接暂时中断，正在重连…（第 2 次，约 2 秒后重试） 原因：网络请求失败：无法连接到服务，请检查服务地址、网络、代理或 TLS 配置。",
        },
      },
    })
  })
...
describe("commands", () => {
  ...
  test("/session shows current session with repo and model", async () => {
    ...
    const ok = await on_cmd("/session", cfg(), route(store), svc, store, ui.api, createRender(), codex(), inbound({ text: "/session" }))
  ...
  test("slash command reuses the same task for resumable follow-up", async () => {
    ...
    expect(await store.list_tasks()).toHaveLength(2)
  ...
  test("/repo shows session over chat and user defaults", async () => {
    ...
    const ok = await on_cmd("/repo", cfg(), route(store), svc, store, ui.api, createRender(), codex(), inbound({ text: "/repo" }))
  ...
  test("/status explains current session and scope context", async () => {
    ...
    const ok = await on_cmd("/status", cfg(), route(store), svc, store, ui.api, createRender(), codex(), inbound({ text: "/status" }))
  ...
  test("/workspaces shows a clear fallback when experimental endpoint is unsupported", async () => {
    ...
    const oc = {
      ...codex(),
      async workspaces() {
        throw new Error("codex request failed: 404 Not Found - /experimental/workspace")
      },
    } satisfies CodexSvc
    ...
    expect(ui.list[0]?.out).toMatchObject({
      kind: "text",
      body: {
        text: "当前 Codex 服务不支持实验接口 /experimental/workspace，暂时无法列出 workspace。",
      },
    })
  })
  ...
  test("/new followed by /sessions renders same-directory remote roots regardless of workspace without marking a pending foreground current", async () => {
    ...
    const ensure_calls: Array<{ directory?: string; workspace?: string; session_id?: string }> = []
    const session_inputs: Array<{ directory?: string; workspace?: string; roots?: boolean; limit?: number }> = []
    const remote_history: CodexSession[] = [
      {
        id: "ses_new",
        title: "New session - 2026-05-06T13:31:05.597Z",
        directory: "/tmp/local-new",
  ...
  test("/new enters pending foreground with chat default scope", async () => {
    ...
    const ok = await on_cmd("/new", cfg(), svc_route, svc, store, ui.api, createRender(), codex(), inbound({ text: "/new" }))
  ...
  test("/model shows current and default model", async () => {
    ...
    const ok = await on_cmd("/model", cfg(), route(store), svc, store, ui.api, createRender(), codex(), inbound({ text: "/model" }))
  ...
  test("/model rejects unavailable bare model", async () => {
    const store = createMemoryStore()
    const svc = createTaskSvc(store)
    const ui = feishu()

    const ok = await on_cmd("/model badformat", cfg(), route(store), svc, store, ui.api, createRender(), codex(), inbound({ text: "/model badformat" }))

    expect(ok).toBeTrue()
    expect(ui.list[0]?.out).toMatchObject({
      kind: "text",
      body: {
        text: "当前没有可用模型：badformat",
      },
    })
  })
  ...
  test("/model reset shows default model for current session", async () => {
    ...
    const ok = await on_cmd("/model reset", cfg(), route(store), svc, store, ui.api, createRender(), codex(), inbound({ text: "/model reset" }))
  ...
  test("/model reset persists the configured default model into session state", async () => {
    ...
    const ok = await on_cmd("/model reset", cfg(), svc_route, svc, store, ui.api, createRender(), codex(), inbound({ text: "/model reset" }))
  ...
  test("waiting_question accepts direct text answer when custom reply is allowed", async () => {
    ...
    await store.save_task(
      row({
        ...
        req_type: "question",
        req: "req_q_1",
        note: `question:1:${encodeURIComponent("要更新哪个仓库的 AGENTS.md？")}:${encodeURIComponent("workspace/codex")}|${encodeURIComponent("workspace/codex-feishu-imui")}`,
      }),
    )
